...
class BattleSystem {
    ...
    autoBattle(autoContinue = false) {
        ...
        // 战斗结束后累计奖励（endBattle 已经处理了经验和金币）
        if (!this.inBattle) {
            const lastRound = rounds[rounds.length - 1];
            if (lastRound && lastRound.result === 'win') {
                const reward = this.getLastBattleResult();
                if (reward) {
        ...
        return { success: true, rounds: rounds.length, expGained, goldGained };
    }
    ...
    endBattle(playerWin) {
        ...
        if (playerWin) {
            ...
            // 存储战斗奖励供 autoBattle 使用
            this.lastBattleReward = {
                exp: expReward,
                gold: goldReward,
    ...
    // 获取最近战斗结果（供 autoBattle 累计使用）
    getLastBattleResult() {
        return this.lastBattleReward;
    }
...
class Game {
    ...
    buyPotion(potionType, count = 1) {
        ...
        const totalCost = potion.cost * count;
        ...
        this.player.potions[potionType] += count;
        this.notifyStateChange();
        
        return {
            success: true,
            message: `购买了 ${potion.icon} ${potion.name} ×${count}，花费 ${totalCost} 金币`,
    ...
    // 设置离线战斗保存回调
    setOfflineFarmSaveCallback(callback) {
        this.onOfflineFarmSave = callback;
    }
