...
function loadGameFromFile(gameId) {
    try {
        ...
        if (fs.existsSync(filePath)) {
            ...
            // 清理可能的中间状态：如果在战斗中，强制结束战斗
            if (game.battleSystem && game.battleSystem.inBattle) {
                console.log(`[Load] Clearing battle state for ${gameId}`);
                game.battleSystem.inBattle = false;
                game.battleSystem.currentEnemy = null;
            }
            
            // 清理离线战斗定时器（需要重新启动）
            if (game.offlineFarmTimer) {
                console.log(`[Load] Clearing offline farm timer for ${gameId}`);
                clearInterval(game.offlineFarmTimer);
                game.offlineFarmTimer = null;
                game.offlineFarmStats = null;
            }
...
io.on('connection', (socket) => {
    ...
    socket.on('buy-potion', (data) => {
        const { potionType, count } = data;
        const result = currentGame.buyPotion(potionType, count || 1);
        if (result.success) {
            saveGameToFile(currentGameId, currentGame);
            io.to(currentGameId).emit('state-update', currentGame.getState());
        }
